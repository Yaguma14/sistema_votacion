...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['agregar_actividad'])) {
    ...
    if ($conn->query($sql) === TRUE) {
        $mensaje = "Actividad recreativa agregada con éxito.";
        crear_notificacion_global("Nueva actividad recreativa: $nombre. ¡No te la pierdas!", "actividad");
        registrar_log($_SESSION['usuario_id'], 'Nueva actividad recreativa', "Actividad: $nombre");
    } else {
        $error = "Error al agregar la actividad: " . $conn->error;
    }
}
